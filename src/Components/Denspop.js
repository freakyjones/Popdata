...
getdata(){
  {this.state.display==="Bar"?this.setState({chartdata: {
    labels: this.state.year,
    datasets: [{
        label: 'increase of population',
        data: this.state.denspop,
  ...
  const interval=setInterval(()=>{
    ...
    .then(data=>{
       ...
       const year=row.map(element=>{
        ...
        const denspop=row.map(element=>{
            return(element.split(',')[9])
        })
        this.setState(()=>{
          const newyear=year;
          const pop=denspop
          ...
          <DataContext.Consumer>
            {
              (context)=>{
                ...
                return(
                  ...
                  <section id="dens" style={{background:`${ Theme.ui_background}`,marginTop:'0px'}}>
